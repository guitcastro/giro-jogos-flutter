...
class _ScoreEditScreenState extends State<ScoreEditScreen> {
  ...
  Stream<ChallengeScore?>? _scoreStream;
  bool _saving = false;
  ...
  Future<void> _initializeData() async {
    ...
    // Create stream once
    _scoreStream = _challengeService!.getScoreStream(
      duoId: widget.duoId,
      challengeId: widget.challengeId,
    );

    // Load challenge and initial score in parallel
    final results = await Future.wait([
      _challengeService!.getChallengeById(int.parse(widget.challengeId)),
      _challengeService!.getScore(
        duoId: widget.duoId,
        challengeId: widget.challengeId,
      ),
    ]);
    ...
    if (mounted) {
      setState(() {
        ...
        final score = results[1] as ChallengeScore?;
        if (score != null) {
  ...
  Widget build(BuildContext context) {
    if (_challenge == null || _scoreStream == null) {
      return Scaffold(
        appBar: AppBar(
                      ...
                      onPressed: _saving
                          ...
                          : () async {
                              ...
                              try {
                                ...
                                await _challengeService!.setScore(
                                  duoId: widget.duoId,
                                  challengeId: widget.challengeId,
                                  points: points,
